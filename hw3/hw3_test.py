...
def training(x,y,val_x,val_y):
	...
	model.add(Conv2D(80, kernel_size=(5, 5), input_shape=x.shape[1:4], padding='valid',kernel_initializer='glorot_normal'))
	model.add(LeakyReLU(1./20))
	model.add(BatchNormalization())
	model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
	model.add(Dropout(0.25))

	model.add(Conv2D(160, kernel_size=(3, 3), padding='same', kernel_initializer='glorot_normal'))
	model.add(LeakyReLU(1./20))
	model.add(BatchNormalization())
	model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
	model.add(Dropout(0.3))

	model.add(Conv2D(500, kernel_size=(5, 5), padding='same',kernel_initializer='glorot_normal'))
	model.add(LeakyReLU(1./20))
	model.add(BatchNormalization())
	model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
	...
	history = model.fit_generator(datagen.flow(x, y, batch_size=128), steps_per_epoch=1000, epochs=8000, validation_data=(val_x, val_y), max_queue_size=100, callbacks=[lr_reducer, earlyStopping, checkpointer])
